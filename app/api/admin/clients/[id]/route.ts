...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { error } = await supabase.from("clients").update({
    full_name: body.fullName, business_name: body.businessName, email: body.email,
    plan: body.plan, status: body.status, credit_addon: body.creditAddon,
    unit_owner: body.unitOwner, unit_number: body.unitNumber,
    mailbox_id: body.mailboxId, notes: body.notes,
